# ------------------------------------------------------------------------------------------------------
# Reference: https://github.com/microsoft/X-Decoder/blob/v2.0/modeling/body/xdecoder_head.py
...
class Pix2CapHead(nn.Module):
    ...
    def forward(self, features, mask=None, target_queries=None, target_densecap=None, task='seg', extra={}):
        return self.layers(features, mask, target_queries, target_densecap, task, extra)

    def layers(self, features, mask=None, target_queries=None, target_densecap=None, task='seg', extra={}):
        mask_features, transformer_encoder_features, multi_scale_features = self.pixel_decoder.forward_features(features)
        
        if self.transformer_in_feature == "multi_scale_pixel_decoder":
            predictions = self.predictor(multi_scale_features, mask_features, mask, target_queries, target_densecap, task, extra)
        else:
...
@register_body
def get_pix2cap_head(cfg, input_shape, lang_encoder, extra):
    return Pix2CapHead(cfg, input_shape, lang_encoder, extra)