...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        keep = (scores > 0.5).cpu()
        boxes = boxes[keep] if predictions.has("pred_boxes") else None
        scores = scores[keep]
        ...
        labels=np.array(predictions.pred_densecaptions)
        labels = labels[np.array(keep)]
    ...
    def draw_dense_panoptic_seg(self, panoptic_seg, segments_info, area_threshold=None, alpha=0.7):
        """
        ...
        for seg in segments_info:
            caption_idx+=1
            caption_pairs.append(f"{caption_idx}:{seg['caption']}\n")
            seg['caption'] = caption_idx     
        ...
        if len(all_instances) == 0:
            return self.output, caption_pairs
        masks, sinfo = list(zip(*all_instances))
        category_ids = [x["category_id"] for x in sinfo]
        densecap = [s['caption'] for s in sinfo]
        ...
        try:
            ...
            colors = None
        self.overlay_instances(masks=masks, labels=densecap, assigned_colors=colors, alpha=alpha)
        return self.output, caption_pairs
